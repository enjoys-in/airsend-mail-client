...
type PrimaryKeyType<T extends keyof Tables> = Tables[T] extends Table<
  infer U,
  infer PK
>
  ? PK
...
type BuildNested<Value, Keys extends string[]> =
...
type NestedToDotPath<T, Prefix extends string = ""> = {
  [K in keyof T & string]:
  T[K] extends object
...
type DotPathToNested<T extends Record<string, any>> = {
...
type OnlyNestedKeys<T, Prefix extends string = ""> = {
...
type PathValue<T, P extends string> = DeepValue<T, Split<P>>;

type UpdatesForTable<T> = {
  [K in NestedKeys<T>]?: PathValue<T, K>;
};

type ParitalUpdatesForTable<T extends keyof Tables> = Partial<{
  [P in NestedKeys<TableValue<Tables[T]>>]: PathValue<TableValue<Tables[T]>, P>;
}>
interface IDatabaseChange<T extends keyof Tables> {
  source: any
  table: string
  key: string
  type: number
  mods: Record<string, any> // updated value
  oldObj: Record<string, any>// old obj of that table
  obj: Record<string, any> // new obj with updated value
  rev: number // revision, migration number  which is key of  previos migration, hepfu for migration
}
type FieldType<
  T extends keyof Tables,
  P extends NestedKeys<TableValue<Tables[T]>>
> = PathValue<TableValue<Tables[T]>, P>;
...
class AirsendDB {
  ...
  async has<T extends keyof Tables, Key extends PrimaryKeyType<T>>(
    table: T,
    key: Key
  ): Promise<boolean> {
    return (await db.table(table).get(key as any)) !== undefined;
  }
  /**
  ...
  >(
    tableName: T,
    primaryKeyValue: PrimaryKeyType<T>, // Primary key is dynamically typed based on the selected table
...
function useDBWithActions<
  T extends keyof Tables,
  K extends NestedKeys<TableValue<Tables[T]>>
>(
  table: T,
  primaryKey: string, // or number
  nestedKey?: K
): {
  data: FieldType<T, K> | undefined;
  update: (value: Partial<PathValue<Tables[T], K> | Tables[T]>) => Promise<void>;
  remove: () => Promise<void>;
} {
  const data = useLiveQuery(() => {
    return db.table(table).get(primaryKey) as Promise<Tables[T] | undefined>;
  }, [table, primaryKey]);
  ...
  const update = async (value: any) => {
    if (!nestedKey) {
      await db.table(table).update(primaryKey, value);
    } else {
      // Build nested object for update
      const nestedUpdate = nestedKey
        .split('.')
        .reverse()
        .reduce((acc, key) => ({ [key]: acc }), value);
      await db.table(table).update(primaryKey, nestedUpdate);
    }
  };

  const remove = async () => {
    await db.table(table).delete(primaryKey);
  };

  const resolved = nestedKey && data ? extractNested(data, nestedKey) : data;

  return {
    data: resolved as FieldType<T, K> | undefined,
    update,
    remove
  };
}
