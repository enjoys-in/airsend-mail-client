import { CacheResponseMap, CacheStorageKey } from "./cache-keys";
...
export class CacheStorageUtil {
  ...
  constructor(cacheName = 'airsend-cache') {
  ...
  async put<K extends CacheStorageKey>(key: K, data: CacheResponseMap[K], ttl?: number) {
    const cache = await this.getCache();
  ...
  async get<K extends CacheStorageKey>(key: K): Promise<CacheResponseMap[K] | null> {
